...
export const BatteryStatusIndicator = GObject.registerClass({
        ...
        this._proxy = new PowerManagerProxy(Gio.DBus.system, BUS_NAME, OBJECT_PATH,
            (proxy, error) => {
                if (error) {
                    ...
                    this._settings.connectObject('changed::scale-battery-level', () => this.sync(), this);
                    this._settings.connectObject('changed::charging-mode', () => this.sync(), this);
                    this.sync();
                }
            });
    ...
    sync() {
        ...
        if(this._settings.get_boolean('scale-battery-level')) {
            let thresholdValue;
            const chargingMode = this._settings.get_string('charging-mode');
            if(chargingMode === 'ful')
                thresholdValue = this._settings.get_int('current-ful-end-threshold')
            if(chargingMode === 'bal')
                thresholdValue = this._settings.get_int('current-bal-end-threshold')
            if(chargingMode === 'max')
                thresholdValue = this._settings.get_int('current-max-end-threshold')
            this._percentage = this._proxy.Percentage * 100 / thresholdValue;
        } else {
            this._percentage = this._proxy.Percentage;
        }
        ...
        const fillLevel = 10 * Math.floor(this._percentage / 10);
        const charged =
        ...
        this._powerToggle.set({
            title: formatter.format(this._percentage / 100),
